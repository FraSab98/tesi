...
def score_cpt(responses: List[CPTResponse]) -> CPTScore:
    ...
    # Commissioni: non-target a cui si è risposto
    commissions = [r for r in non_targets if r.responded]
    n_commissions = len(commissions)

    # Tutti i RT validi (target colpiti + commissioni), non solo i target corretti
    all_response_rts = [
    ...
    # Variabilità calcolata su TUTTE le risposte (più sensibile)
    ...
    # "Instabilità attentiva" combinata: variabilità RT + tasso di lapsus
    attention_instability = rt_variability + (omission_rate * 0.5)

    # Score composito: 100 - penalità
    ...
    # Pesi basati su letteratura: omissione più grave di commissione per attenzione sostenuta
